...
When any `after_hook` block is configured, the `PIPELINES_PLAN_ENCRYPTION_KEY` secret must be set. Pipelines will fail preflight checks if hooks are declared and this secret is missing.
...
```hcl
execute = ["bash", "-c", "echo \"Triggered by $PIPELINES_HOOKS_CTX_ACTOR\""]
```
