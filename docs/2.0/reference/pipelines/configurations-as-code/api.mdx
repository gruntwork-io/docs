...
<HclListItemDescription>
Whether the hook runs even when the preceding `plan` or `apply` command fails, or when an earlier hook in the run failed. When `false`, the hook is skipped if anything before it failed.
</HclListItemDescription>
<HclListItemDefaultValue defaultValue="false"/>
</HclListItem>
