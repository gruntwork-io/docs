...
| `PIPELINES_HOOK_CTX_ACTOR` | The user that triggered the run. |
| `PIPELINES_HOOK_CTX_GIT_REF` | The git ref that triggered the run. |
| `PIPELINES_HOOK_CTX_GIT_HASH` | The commit SHA the run is operating on. |

### Set when applicable

| Variable | Description |
|---|---|
| `PIPELINES_HOOK_CTX_ACTION` | The command the hook ran after: `plan` or `apply`. A destroy is reported as `apply`. |
| `PIPELINES_HOOK_CTX_ACTION_STATUS` | The outcome of that command: `succeeded` or `failed`. |
...
| Variable | Description |
|---|---|
| `PIPELINES_HOOK_IN_PLAN_JSON_DIR` | Directory containing the decrypted plan JSON for the run's units. |
| `PIPELINES_HOOK_IN_UNITS_JSON_FILE` | Path to a JSON file describing the units in the run. |
