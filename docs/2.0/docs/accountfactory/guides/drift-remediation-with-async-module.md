...
|Component                    | Role |
| --                          | -- |
|EventBridge Rule             | Listens for Service Catalog API calls like UpdateProvisioningArtifact and UpgradeProduct
...
|Step Function State Machine  | Monitors the update process and confirms success or failure
...
flowchart TD
    ...
    %% Trigger & Event Rule
    A[User/API triggers UpdateProvisioningArtifact or UpgradeProduct]
    ...
    F -->|StartExecution| H[Step Function]

    %% Worker exits
    F --> X((Lambda Ends))

    %% Step Function handles status polling
...
- Drift is resolved outside of OpenTofu/Terraform
...
After apply, drifted `provisioned_product_id` values will be detected and remediated any time [UpdateProvisioningArtifact](https://docs.aws.amazon.com/servicecatalog/latest/dg/API_UpdateProvisioningArtifact.html) or [UpdateProvisionedProduct](https://docs.aws.amazon.com/servicecatalog/latest/dg/API_UpgradeProduct.html) API calls are detected. 

_Note: If your environment is already in a drifted state, you will need to trigger UpdateProvisioningArtifact or UpdateProvisionedProduct to initiate drift remediation. The simplest way to do this is to deactive and reactivate the current provisioned product version._
