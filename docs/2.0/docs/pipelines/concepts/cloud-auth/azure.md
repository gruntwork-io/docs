...
environment "my_azure_subscription" {
  ...
  authentication {
    azure_oidc {
      ...
      # cloud_environment defaults to "public"; set to "usgovernment", "china",
      # "german", or "stack" to target an Azure sovereign cloud.
    }
  }
}
```
...
By default, Pipelines targets the public Azure cloud (`login.microsoftonline.com` / `management.azure.com`). To target an Azure sovereign cloud, set the `cloud_environment` attribute on the `azure_oidc` block. Pipelines will then export `ARM_ENVIRONMENT` to Terragrunt alongside the other `ARM_*` variables, and the azurerm provider will use the matching set of cloud endpoints (for example, `login.microsoftonline.us` / `management.usgovcloudapi.net` for Azure Government).

```hcl title=".gruntwork/environments.hcl"
environment "my_azure_gov_subscription" {
  filter {
    paths = ["my-azure-gov-subscription/*"]
  }

  authentication {
    azure_oidc {
      tenant_id         = "a-gov-tenant-id"
      subscription_id   = "a-gov-subscription-id"
      plan_client_id    = "plan-client-id"
      apply_client_id   = "apply-client-id"
...
Accepted values (lowercase, matching the azurerm provider): `public`, `usgovernment`, `china`, `german`, `stack`.

To make local Terragrunt runs target the same cloud as Pipelines, pass the same value to the `environment` attribute of your `azurerm` and `azuread` provider blocks (typically generated by your `root.hcl`). The OpenTofu/Terraform state backend (`backend "azurerm"`) must also be configured for the target cloud.

For end-to-end setup walkthroughs, see:

- The [Connecting to Azure Government](/2.0/docs/pipelines/tutorials/deploying-to-azure-government) tutorial.
- Microsoft's [sovereign cloud endpoint reference](https://learn.microsoft.com/en-us/azure/azure-government/documentation-government-developer-guide).
- The azurerm provider's [`environment`](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs#environment) documentation.
