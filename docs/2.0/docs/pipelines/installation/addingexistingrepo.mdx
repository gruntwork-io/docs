...
<PersistentCheckbox id="update-azure-location" label="Update the location in the bootstrap configuration to the location you want to bootstrap your subscription in." />
...
<PersistentCheckbox id="set-azure-unique-storage-names" label="Set a globally unique storage account name for the storage account in each subscription." />
...
1. Set the environment variables used by the Azure provider to authenticate in a given subscription:
...
<PersistentCheckbox id="run-azure-bootstrap-plan" label="Run a plan in the bootstrap directory to make sure that everything is set up correctly." />
