...
<PersistentCheckbox id="set-subscription-name" label="Create a new directory for the subscription you want to bootstrap, named after the subscription." />
...
<PersistentCheckbox id="set-storage-account-name" label="Set the name of the storage account you want to use for state storage in the `sub.hcl` file." />
...
stack "bootstrap" {
  source = "git@github.com:gruntwork-io/terragrunt-scale-catalog.git//stacks/azure/bootstrap?ref=v1.0.0"
  path   = "bootstrap"

  values = {
    terragrunt_scale_catalog_url = "git@github.com:gruntwork-io/terragrunt-scale-catalog.git"
    ...
    // Set the organization name you want to use for your subscription.
    org_name = "acme"

    // Set the repository name you want to use for your subscription.
    repo_name = "infrastructure-live"
...
<PersistentCheckbox id="set-org-name" label="Set the organization name in the `terragrunt.stack.hcl` file to the GitHub Organization name you want Pipelines to trust." />
<PersistentCheckbox id="set-repo-name" label="Set the repository name in the `terragrunt.stack.hcl` file to the name of the repository you want Pipelines to trust." />
...
If you haven't already, you'll want to authenticate with Azure using the `az` CLI.
...
If you encounter issues during this step, please refer to the [Initial Apply Failure](#initial-apply-failure) section.
...
<PersistentCheckbox id="pull-resources-into-state" label="Pull the resources into state using Terragrunt." />
...
<PersistentCheckbox id="check-initial-apply-fails" label="Read the error message in its entirety." />
<PersistentCheckbox id="check-initial-apply-fails" label="Confirm that you ran the terragrunt apply command with all the required flags: `terragrunt run --all --non-interactive --provider-cache --no-stack-generate apply`" />
<PersistentCheckbox id="check-initial-apply-fails" label="Confirm that you have the correct Azure credentials and permissions to create resources in your subscription" />
<PersistentCheckbox id="check-initial-apply-fails" label="Confirm that you have exported the ARM_TENANT_ID and ARM_SUBSCRIPTION_ID environment variables if you haven't set them via the `az` CLI" />
<PersistentCheckbox id="check-initial-apply-fails" label="Confirm that your storage account name in sub.hcl is unique across all of Azure and follows naming requirements (less than 24 chars, lowercase letters and numbers only)" />
<PersistentCheckbox id="check-initial-apply-fails" label="Ensure your location value in terragrunt.stack.hcl matches an actual Azure region name (e.g. 'East US', not 'eastus')" />
