...
To configure Pipelines in a new GitHub repository, complete the following steps (which are explained in detail below):
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/aws/github/infrastructure-live?ref=v1.13.1' \
  --output-folder .
```
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/azure/github/infrastructure-live?ref=v1.13.1' \
  --output-folder .
```
...
The resources that you need provisioned in GCP to start managing resources with Pipelines are:
...
For every GCP project you want Pipelines to manage infrastructure in.
...
1. Set up the Terragrunt configurations in your `infrastructure-live` repository for bootstrapping Pipelines in a single GCP project
2. Use Terragrunt to provision these resources in your GCP project
3. (Optionally) Bootstrap additional GCP projects until all your GCP projects are ready for Pipelines
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/gcp/github/infrastructure-live?ref=v1.13.1' \
  --output-folder .
```
...
Once you've set up the Terragrunt configurations, you can use Terragrunt to provision the resources in your GCP project.
...
First, make sure that everything is set up correctly by running a plan in the `bootstrap` directory in `project-name` where `project-name` is the name of the first GCP project you want to bootstrap.
...
<PersistentCheckbox
  id="run-gcp-apply"
  label="Run an apply in the bootstrap directory to provision the resources in your GCP project."
/>
...
<h3>Optional: Bootstrapping additional GCP projects</h3>

If you have multiple GCP projects, and you want to bootstrap them as well, you can do so by following a similar, but slightly condensed process.

For each additional project you want to bootstrap, you'll use Boilerplate in the root of your `infrastructure-live` repository to scaffold out the necessary content for just that project.

:::tip

If you are going to bootstrap more GCP projects, you'll probably want to commit your existing changes before proceeding.
...
Just like before, you'll use Boilerplate to scaffold out the necessary content for just that project.
...
boilerplate \
  ...
  --var 'GCPProjectID=my-prod-project-456' \
  --var 'GCPProjectNumber=456789012345' \
  --var 'GCPRegion=us-central1' \
  --var 'GitHubOrgName=acme' \
  --var 'GitHubRepoName=infrastructure-live' \
  --var 'GitHubOrgID=1234567' \
...
```yaml title="vars.yml"
ProjectName: prod
GCPProjectID: my-prod-project-456
GCPProjectNumber: "456789012345"
GCPRegion: us-central1
GitHubOrgName: acme
GitHubRepoName: infrastructure-live
GitHubOrgID: 1234567
...
<PersistentCheckbox
  id="scaffold-additional-project"
  label="Use Boilerplate to scaffold out additional GCP projects."
/>

:::

Once you've scaffolded out the additional projects you want to bootstrap, you can use Terragrunt to provision the resources in each of these projects.

:::tip

Make sure that you authenticate to GCP with an account that has permissions in each GCP project you are bootstrapping before you attempt to provision resources in it.
...
<PersistentCheckbox
  id="run-additional-gcp-plan"
  label="Run a plan in the bootstrap directory of the additional project to make sure that everything is set up correctly."
/>
<PersistentCheckbox
  id="run-additional-gcp-apply"
  label="Run an apply in the bootstrap directory of the additional project to provision the resources in the additional project."
/>
...
🚀 You've successfully added Pipelines to your new repository!

## Next steps

You have successfully completed the installation of Pipelines in a new repository. Proceed to [Deploying your first infrastructure change](/2.0/docs/pipelines/tutorials/deploying-your-first-infrastructure-change) to begin deploying changes.
