...
Use the Gruntwork.io GitHub App to [add the repository as an Infra Root repository](/docs/terragrunt-scale/pipelines/installation/viagithubapp#configuration).

If using the [machine user model](/docs/terragrunt-scale/pipelines/installation/viamachineusers), ensure the `INFRA_ROOT_WRITE_TOKEN` (and `ORG_REPO_ADMIN_TOKEN` for enterprise customers) is added to the repository as a secret or configured as an organization secret.
...
:::info
To use Gruntwork Pipelines in an **existing** GitLab repository, see this [guide](/docs/terragrunt-scale/pipelines/installation/addinggitlabrepo).
:::
...
- Completed the [AWS Landing Zone setup](/docs/aws-accelerator/account-factory/prerequisites/awslandingzone)
- Have programmatic access to the AWS accounts created in the [AWS Landing Zone setup](/docs/aws-accelerator/account-factory/prerequisites/awslandingzone)
- Completed the [Pipelines Auth setup for GitLab](/docs/terragrunt-scale/pipelines/installation/viamachineusers#gitlab) and setup a machine user with appropriate PAT tokens
...
This is necessary because Gruntwork Pipelines uses [GitLab CI/CD components](/docs/terragrunt-scale/pipelines/architecture/ci-workflows), and GitLab requires components to reside within the [same GitLab instance as the project referencing them](https://docs.gitlab.com/ci/components/#use-a-component).
...
The `infrastructure-catalog` repository is a collection of modules that can be used to build your infrastructure. It is a great way to share modules with your team and across your organization.
