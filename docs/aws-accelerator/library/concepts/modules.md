...
Consider the [`rds` module](https://library.gruntwork.io/modules/terraform-aws-data-storage/rds). This OpenTofu/Terraform module creates an RDS database, the IAM roles required to operate it, optional read replicas, database subnet groups, and relevant security groups.
...
To explore complete solutions combining building blocks, refer to [Service Modules](/docs/aws-accelerator/library/concepts/service-modules).
...
Consider [Service Modules](/docs/aws-accelerator/library/concepts/service-modules) when optimizing for greater convenience.
...
We recommend using the [Service Catalog](/docs/aws-accelerator/library/concepts/service-modules) for everyday use cases, with the entire module catalog available for more complex needs.
...
To browse the module catalog, see the [Library Reference](https://library.gruntwork.io) or peruse the [private Gruntwork GitHub repositories using your subscription](https://github.com/orgs/gruntwork-io/repositories?q=&type=private&language=&sort=).
...
Refer to [Gruntwork releases](/docs/release-notes/) for a comprehensive listing of updates.
