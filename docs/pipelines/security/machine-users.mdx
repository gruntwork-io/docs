...
:::info
Fine grained tokens are only required for users with a [Gruntwork DevOps Foundations setup](../../foundations/overview) for use with the Landing Zone(Account Factory) component.
:::

<details><summary><strong>Details specific to Gruntwork DevOps Foundations setup</strong></summary>
...
The recommended setup for Pipelines uses a machine user that only has read access to repositories in GitHub referred to as the `ci-read-only-user`.

:::info
Note that additional licenses in your GitHub subscription might be required if you are at capacity.

Also a second Github user(`ci-user`) with the ability to open pull requests and run workflows is recommended for integrating with the Account Factory component of the [Gruntwork DevOps Foundations setup](../../foundations/overview).
:::

Restrictive permissions are then granted to the Github user(s), granting them requisite permissions to accomplish their tasks. This means that in order to actually run a pipeline job, both users may be involved at separate stages.

While we refer to these users as `ci-read-only-user` and `ci-user`, but you may name them anything you like. Ensure that the user(s) are:

1. A member of your GitHub Organization
2. A member of your team in **Gruntwork**’s GitHub Organization (See [instructions on inviting a user to your team](https://docs.gruntwork.io/developer-portal/invite-team#inviting-team-members) and [linking the user’s GitHub ID to Gruntwork](https://docs.gruntwork.io/developer-portal/link-github-id))

### ci-read-only-user

This user is created to download private software within GitHub Actions Workflows. The `ci-read-only-user` is used to access the following:

- Gruntwork Library modules & Pipelines CLI
- Your infrastructure repository and
- Any other custom module repositories that are private and used in your infrastructure repository
...
:::info
We recommend creating a second machine user for integrating with the Account Factory component of the [Gruntwork DevOps Foundations setup](../../foundations/overview) but you can adjust this based on your organization's needs and ensure that the correct tokens are created and that the permissions for the machine users and tokens are appropriate for their required access.
:::

<details><summary><strong>Details specific to Gruntwork DevOps Foundations setup</strong></summary>

:::caution
I you decide against creating a second user, be aware that there is a security implication to having one machine user for all the tokens used by Pipelines, as it would need to have write access to all repositories that the `ci-user` and `ci-read-only-user` need to access. This violates the security [principle of least privilege](https://en.wikipedia.org/wiki/Principle_of_least_privilege) access.
...
<details><summary>Instructions for setup <strong>without Gruntwork DevOps Foundations</strong></summary>

<Tabs groupId="github-actions-secrets-non-devops-foundations">
...
3. Using the **New organization secret** option, add the following a secret:
...
<details><summary>Instructions for setup with Gruntwork DevOps Foundations</summary>
...
<!-- ##DOCS-SOURCER-START
{
  ...
  "hash": "1dd42f9f818fe74fd927e4d0f3611253"
}
##DOCS-SOURCER-END -->
