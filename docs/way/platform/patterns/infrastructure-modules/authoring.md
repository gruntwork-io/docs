...
3. **Use commercial infrastructure modules** - You purchase a commercial infrastructure module library like the [Gruntwork AWS IaC Library](https://docs.gruntwork.io/docs/aws-accelerator/library/concepts/overview). This gives you moderate control (you can engage the vendor if needed) and outsources maintenance to others (moderate control, high convenience).
...
1. **Direct reference** - You decide that users will directly reference the infrastructure module. For example, you choose a commercial infrastructure module and decide your [infrastructure module instances](/docs/way/platform/patterns/types#infrastructure-module-instances) will directly reference the vendor's GitHub repo. 
...
:::note
You can see a real-world example of the low-level/high-level split in the [Gruntwork AWS IaC Library](https://docs.gruntwork.io/docs/aws-accelerator/library/concepts/overview#two-types-of-modules), which has both "building block modules" (low-level modules) and "service modules" (high-level modules).
:::
...
- **Use automation** - Tools like [Terragrunt's dependency management](https://terragrunt.gruntwork.io/docs/features/keep-your-terraform-code-dry/) or an [IaC updater](/docs/way/platform/components/maintain/iac-updater) can help automate the process of updating module versions across your infrastructure.
