---
hide_table_of_contents: true
hide_title: true
---

import Card from "/src/components/Card"
import CardGroup from "/src/components/CardGroup"
import CenterLayout from "/src/components/CenterLayout"

<CenterLayout>

# Gruntwork Products

<CardGroup cols={2}>

<Card
  title="AWS Accelerator"
  href="/2.0/docs/overview/concepts/gruntworkplatform">
A comprehensive solution that combines the AWS IaC Library, AWS Account Factory, and AWS Platform Architecture with Terragrunt Scale to accelerate your cloud infrastructure.
</Card>
<Card
  title="Terragrunt Scale"
  href="/2.0/docs/pipelines/concepts/overview">
Deploy infrastructure at scale with Gruntwork Pipelines, Drift Detection, and Patcher—tools that simplify CI/CD for infrastructure code.
</Card>
<Card
  title="AWS IaC Library"
  href="/2.0/docs/library/concepts/overview">
A collection of reusable Infrastructure as Code modules that enables you to deploy and manage AWS infrastructure quickly and reliably.
</Card>
<Card
  title="AWS Account Factory"
  href="/2.0/docs/accountfactory/concepts/">
Create and manage AWS accounts with best-practice baselines, security configurations, and automated provisioning.
</Card>

</CardGroup>

</CenterLayout>


<!-- ##DOCS-SOURCER-START
{
  "sourcePlugin": "local-copier",
  "hash": "3bbe852b160e129b54c8e1a7bd67b2a7"
}
##DOCS-SOURCER-END -->
