...
Pipelines v3 automatically supports Terragrunt Stacks as long as you're using Terragrunt `v0.71.3` or higher. You can specify the Terragrunt binary version in the `mise.toml` file in your repository, see the [configuration reference](/docs/terragrunt-scale/pipelines/reference/configurations#example-mise-configuration).

See [Terragrunt Version Compatibility](/docs/terragrunt-scale/pipelines/reference/terragrunt-version-compatibility) for more details on specific Terragrunt versions and their compatibility with Pipelines.
