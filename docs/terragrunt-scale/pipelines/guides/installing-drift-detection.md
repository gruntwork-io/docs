...
Verify that the [GitHub App](/docs/terragrunt-scale/pipelines/installation/viagithubapp) is installed and in use for this repository. Drift Detection relies on permissions granted by the GitHub App and is not compatible with machine user tokens.
...
Verify that your GitLab project has the necessary [Machine User tokens](/docs/terragrunt-scale/pipelines/installation/viamachineusers) configured. Drift Detection requires:
...
Follow the instructions at [Running Drift Detection](/docs/terragrunt-scale/pipelines/guides/running-drift-detection) to start using the new workflow.
