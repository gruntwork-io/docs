...
In order to ensure that the availability of the Gruntwork.io GitHub App is not something that can impair the ability of users to drive infrastructure updates, users can also authenticate with GitHub using [Machine users](/docs/terragrunt-scale/pipelines/installation/viamachineusers).
...
For more information, see the [relevant architecture documentation](/docs/terragrunt-scale/pipelines/architecture/#infrastructure-live-root).
