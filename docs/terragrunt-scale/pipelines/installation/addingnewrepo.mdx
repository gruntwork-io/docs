...
If you are using the [Gruntwork Account Factory](/docs/aws-accelerator/account-factory/architecture), this will be done automatically during onboarding and in the process of [vending every new AWS account](/docs/aws-accelerator/account-factory/guides/vend-aws-account), so you don't need to worry about this.
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/aws/github/infrastructure-live?ref=v1.3.2' \
  --output-folder .
```
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/azure/github/infrastructure-live?ref=v1.3.2' \
  --output-folder .
```
...
You have successfully completed the installation of Gruntwork Pipelines in a new repository. Proceed to [Deploying your first infrastructure change](/docs/terragrunt-scale/pipelines/tutorials/deploying-your-first-infrastructure-change) to begin deploying changes.
