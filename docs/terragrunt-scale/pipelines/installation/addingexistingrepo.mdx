...
1. Using the [Gruntwork.io GitHub App](/docs/terragrunt-scale/pipelines/installation/viagithubapp#configuration) (recommended for most GitHub users).
2. Using a [machine user](/docs/terragrunt-scale/pipelines/installation/viamachineusers) (recommended for GitHub users who cannot use the GitHub App).
...
If you have some of them provisioned, but not all, you can decide to either destroy the resources you already have provisioned and recreate them or import them into state. If you are not sure, please contact [Gruntwork support](/docs/support).
...
If you don't have a `root.hcl` file, you might need to customize the bootstrapping process, as the Terragrunt scale catalog expects a `root.hcl` file in the root of the repository. Please contact [Gruntwork support](/docs/support) for assistance if you need help.
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/aws/github/account?ref=v1.3.2' \
  --output-folder .
```
...
```bash
boilerplate \
  --template-url 'github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/azure/github/subscription?ref=v1.3.2' \
  --output-folder .
```
...
Create [HCL configurations](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/) in the `.gruntwork` directory in the root of your repository to tell Pipelines how you plan to organize your infrastructure, and how you plan to have Pipelines authenticate with your cloud provider(s).
...
Next, you'll want to define the environments you want to manage with Pipelines using the [`environment` block](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#environment-block).

For each environment, you'll want to define a [`filter` block](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#filter-block) that tells Pipelines which units are part of that environment. You'll also want to define an [`authentication` block](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#authentication-block) that tells Pipelines how to authenticate with your cloud provider(s) for that environment.
...
Learn more about how Pipelines authenticates to AWS in the [Authenticating to AWS](/docs/terragrunt-scale/pipelines/concepts/cloud-auth/aws) page.

:::

:::tip

Check out the [aws block](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/#aws-blocks) for more information on how to configure Pipelines to reuse common AWS configurations.
...
Learn more about how Pipelines authenticates to Azure in the [Authenticating to Azure](/docs/terragrunt-scale/pipelines/concepts/cloud-auth/azure) page.
...
Learn more about how Pipelines can authenticate with custom authentication in the [Custom Authentication](/docs/terragrunt-scale/pipelines/concepts/cloud-auth/custom) page.
...
You have successfully completed the installation of Gruntwork Pipelines in an existing repository. Proceed to [Deploying your first infrastructure change](/docs/terragrunt-scale/pipelines/tutorials/deploying-your-first-infrastructure-change) to begin deploying changes.
