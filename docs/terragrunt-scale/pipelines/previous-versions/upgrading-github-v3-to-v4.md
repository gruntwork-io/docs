...
### Azure Support
Pipelines v4 includes support for Azure OIDC and state storage.  New configuration options are now available in [HCL configuration](https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#azure_oidc-block-attributes) for Azure.
...
Pipelines v4 includes [a more expressive syntax](https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/guides/running-drift-detection#drift-detection-filter) for filters when triggering drift detection.

### Removal of several feature flags
The following [feature flags](https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/reference/feature-flags) are now all *enabled* by default:
...
[Repository Block Attributes](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#repository-block-attributes)
to find which of your existing YML configurations need to be added here.

```hcl title=".gruntwork/repository.hcl"
# Configurations applicable to the entire repository, see: https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#repository-block
repository {
...
# AWS account configurations that are usable by the entire repository, see: https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#aws-block
aws {
  accounts "all" {
...
# Configurations that are applicable to a specific environment within a repository, see: https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api/#environment-block
...
See the [Account Factory HCL](/docs/aws-accelerator/account-factory/reference/configurations-as-code) for
a full reference of values or contact [Gruntwork Support](mailto:support@gruntwork.io) for assistance.

For Enterprise customers using Account Factory: see the [Account Vending Configuration](/docs/aws-accelerator/account-factory/reference/configurations-as-code#account_vending-block) for converting the account vending configuration to HCL.
  ...
  workflow_dispatch:
    inputs:
      pipelines_drift_detection_filter:
        description: Limit drift detection to units matching filter https://docs.gruntwork.io/docs/terragrunt-scale/pipelines/guides/running-drift-detection#drift-detection-filter
        type: string
...
Note that the syntax for the filter input in Drift Detection has changed. Refer to the [Filter Reference](/docs/terragrunt-scale/pipelines/guides/running-drift-detection#drift-detection-filter) for a full description of the new and expanded capabilities.
...
- [INFRA_ROOT_WRITE_TOKEN](/docs/terragrunt-scale/pipelines/installation/viamachineusers#infra_root_write_token) and
- [ORG_REPO_ADMIN_TOKEN](/docs/terragrunt-scale/pipelines/installation/viamachineusers#org_repo_admin_token)
...
Customers using the [Gruntwork.io GitHub App](/docs/terragrunt-scale/pipelines/installation/viagithubapp#gruntworkio-github-app) should also update the above permissions so that the tokens already have the necessary permissions when used as a fallback mechanism.
