...
- Pipelines installed in a GitHub or GitLab repository. Refer to [Setup & Installation](/docs/terragrunt-scale/pipelines/installation/overview) for more details.
- Familiarity with Pipelines Configuration as Code (HCL). Refer to the [Configurations as Code](/docs/terragrunt-scale/pipelines/reference/configurations-as-code) documentation for more details.
...
1. The `aws-us-gov` partition must be present in the ARN for the plan/apply roles configured in [aws_oidc](/docs/terragrunt-scale/pipelines/reference/configurations-as-code/api#aws_oidc-block-attributes) block for the account, typically in the `.gruntwork/`<CustomizableValue id="ACCOUNT_NAME"/>.hcl file.
...
terraform {
 source = "github.com/gruntwork-io/terragrunt-scale-catalog//modules/aws/s3-bucket?ref=v1.3.2"
}
