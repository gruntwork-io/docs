...
As we mentioned in the [Patcher overview](/docs/terragrunt-scale/patcher/concepts/index.md), Patcher supports two primary modes of working:
...
:::info
Individual pull requests can quickly get overwhelming. Once you've got this working, check out our page on [grouping](/docs/terragrunt-scale/patcher/concepts/grouping) to see how to consolidate many updates into a single pull request.
:::
...
Many GitHub Enterprise users self-host GitHub Enterprise and wish to host all binaries internally. If you'd like to fully [self-host Patcher](/docs/terragrunt-scale/patcher/guides/self-hosting), then follow the steps below to set up ongoing updates:
...
If you need to promote updates across multiple environments (dev → stage → prod) rather than updating everything at once, see the [Setting up Promotion Workflows](/docs/terragrunt-scale/patcher/guides/promotion-workflows) guide. That approach provides:
