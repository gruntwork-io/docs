...
To support this asynchronous pattern, this module uses a lifecycle block to ignore changes to the `provisioning_artifact_id` attribute of the underlying Service Catalog `aws_servicecatalog_provisioned_product` resource. This prevents Terraform/OpenTofu from trying to update the resource directly, which would otherwise result in a long-running operation or potential failure.
...
This module is designed to not directly apply changes when the `provisioned_product_id` changes for an account. Instead, it will capture the current and new values and output them as `current_provisioning_artifact_id` and `new_provisioning_artifact_id` respectively. The expectation is that an external service can then use these to update each account to the new value as desired. A practical example of this can be seen in the `control-tower-multi-account-factory-async` module.
...
<!-- ##DOCS-SOURCER-START
{
  ...
  "hash": "df57ceb2bd32b589be115eb2eff5eff4"
}
##DOCS-SOURCER-END -->
