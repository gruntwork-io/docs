...
This module is designed to not directly apply changes when the `provisioned_product_id` changes for an account.
Instead, it will capture the current and new values and output them as `current_provisioning_artifact_id`
and `new_provisioning_artifact_id` respectively. The expectation is that an external service can then use these
to update each account to the new value as desired. A practical example of this can be seen in the
`control-tower-multi-account-factory-async` module.
...
<!-- ##DOCS-SOURCER-START
{
  ...
  "hash": "17aecc4e05c379c8121f536563e746a8"
}
##DOCS-SOURCER-END -->
