...
func (d *ModuleExampleDoc) getRelOutputPath() (string, error) {
	...
	if len(submatches[0]) != IS_MODULE_EXAMPLE_DOC_REGEX_NUM_CAPTURE_GROUPS + 1 {
		return outputPath, errors.New("Module Example Documents must exist in the path /packages/<package-name>/examples/<module-name>/<doc-name>.md Any subfolders in /_docs will generate an error.")
	}
